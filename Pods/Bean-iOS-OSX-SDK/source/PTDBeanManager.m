...
-(void)startScanningForBeans_error:(NSError**)error{
    ...
    for( CBPeripheral* beanPeripheral in connectedBeanPeripherals){
        ...
        if((bean = [beanRecords objectForKey:beanPeripheral.identifier])){
            [self __notifyDelegateOfDiscoveredBean:bean error:nil strength:nil];
...
-(void)__notifyDelegateOfDiscoveredBean:(PTDBean*)bean error:(NSError*)error strength:(NSNumber*)RSSI{
    PTDLog(@"[DEBUG] notifying delegate of discovered bean");
    //Deprecated
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    if (self.delegate && [self.delegate respondsToSelector:@selector(BeanManager:didDiscoverBean:error:)]){
        [self.delegate BeanManager:self didDiscoverBean:bean error:error strength:RSSI];
    }
#pragma clang diagnostic pop
    if (self.delegate && [self.delegate respondsToSelector:@selector(beanManager:didDiscoverBean:error:strength:)]){
        [self.delegate beanManager:self didDiscoverBean:bean error:error strength:RSSI];
    } else {
        PTDLog(@"[DEBUG] actually not gonna notify");
    }
}
...
-(void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI{
    ...
    if(bean){
        ...
        [self __notifyDelegateOfDiscoveredBean:bean error:nil strength:RSSI];
    }
}
